...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  await Promise.all(
    // save all the img processing promises to an array
...
exports.getTourStats = catchAsync(async (req, res, next) => {
  const stats = await Tour.aggregate([
    /* 1st pipeline */
    {
      // match: select or filter certain documents
      $match: {
    ...
    {
      $group: {
        // group by difficulty (in uppercases)
        _id: { $toUpper: 'difficulty' }, // if _id: null, then group by nothing
        numTours: { $sum: 1 },
...
exports.getMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  const plan = await Tour.aggregate([
    {
      $unwind: '$startDates' // unnest the startDates array
    },
    {
      $match: {
        startDates: {
    ...
    {
      $sort: {
        numTourStarts: -1 //descending order
      }
    },
    {
      $limit: 12
...
exports.getDistances = catchAsync(async (req, res, next) => {
  ...
  const distance = await Tour.aggregate([
    {
      // this operator will compute the distance and insert a distanceField into the original payload
      $geoNear: {
        near: {
          type: 'Point',
    ...
    {
      // this operator filters the fields that only stated in the values
      $project: {
        distance: 1, // 1 means ascending order
        name: 1
      }
    }
  ])
